...
Functions are tested self-consistently against each other.
"""
...
def for_back_torch_2d():
    ...
    for shape in all_shapes:
        ...
        assert (max_diff0 <= eps and max_diff1 <= eps), f"Max errors: \
            {max_diff0}, {max_diff1}"
        assert len(grid_r) == 2
        assert abs(atoms[0] - atoms[1]) <= 10 * eps * math.prod(shape), \
...
def compare_torch_1d_2d():
    ...
    for shape in all_shapes:
        ...
        assert abs(atoms[0] - atoms[1]) <= 10 * eps * math.prod(shape), \
            ...
            \nDifference: {abs(atoms[0] - atoms[1])}; \
            \n2*eps*N: {eps * math.prod(shape)}."
    print("Test `compare_torch_1d_2d` passed.")


def compare_np_1d_2d():
    """Compute two forward 1D FFTs and then a single inverse 2D FFT.
    ...
    for shape in all_shapes:
        ...
        assert abs(atoms[0] - atoms[1]) <= 10 * eps * math.prod(shape), \
            ...
            \nDifference: {abs(atoms[0] - atoms[1])}; \
            \n2*eps*N: {eps * math.prod(shape)}."
    print("Test `compare_np_1d_2d` passed.")
...
def compare4_torch_1d_2d():
    ...
    for shape in all_shapes:
        grid = [func(shape, dtype=dtype, device='cuda')] * 2
        grid_r_half = ttools.ifft_1d(grid, delta_r, axis=1)
        grid_r = ttools.ifft_1d(grid_r_half, delta_r, axis=0)
        ...
        assert (max_diff0 <= 10 * eps and max_diff1 <= 10 * eps), f"Max errors: \
            {max_diff0}, {max_diff1}."
        assert len(grid_r) == 2
        assert abs(atoms[0] - atoms[1]) <= 10 * eps * math.prod(shape), \
            f"\nAtom num. before/after: {atoms[0]}, {atoms[1]}; \
...
if __name__ == "__main__":
    ...
    # compare_torch_1d_2d()  # 1D forward (x2) & 2D back, pytorch
    # compare_np_1d_2d()  # 1D forward (x2) & 2D back, numpy
    ...
    # These comparing the inverse were not as accurrate at the forward ones.
    compare4_torch_1d_2d()  # 2D back & 1D back (x2), pytorch
    compare4_np_1d_2d()  # 2D back & 1D back (x2), numpy

# FFT functions should:
# - Preserve normalization/atom number
# - Forward and then inverse FFT should return the original
